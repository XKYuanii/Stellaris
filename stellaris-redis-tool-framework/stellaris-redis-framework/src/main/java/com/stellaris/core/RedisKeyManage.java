...
public enum RedisKeyManage {
    ...
    PROGRAM("program_%s","节目id","节目","k"),
    
    PROGRAM_GROUP("program_group_%s","节目分组id","节目分组","k"),
    
    PROGRAM_SHOW_TIME("program_show_time_%s","节目演出时间id","节目演出时间","k"),
    
    PROGRAM_SEAT_NO_SOLD_RESOLUTION_HASH("program_seat_no_sold_resolution_hash_%s_%s","节目座位未售卖集合_节目id_节目类型id","节目座位未售卖集合","k"),
    
    PROGRAM_SEAT_LOCK_RESOLUTION_HASH("program_seat_lock_resolution_hash_%s_%s","节目座位锁定集合_节目id_节目类型id","节目座位锁定集合","k"),
    
    PROGRAM_SEAT_SOLD_RESOLUTION_HASH("program_seat_sold_resolution_hash_%s_%s","节目座位已售卖集合_节目id_节目类型id","节目座位已售卖集合","k"),
    
    PROGRAM_TICKET_CATEGORY_LIST("program_ticket_category_list_%s","节目票档集合id","节目票档集合","k"),
    
    PROGRAM_TICKET_REMAIN_NUMBER_HASH_RESOLUTION("program_ticket_remain_number_hash_resolution_%s_%s","节目余票数量_节目id_节目票档id","节目余票数量","k"),
    
    PROGRAM_CATEGORY_HASH("program_category_hash","节目类型hash集合","节目类型hash集合","k"),
    
    PROGRAM_RECORD("program_record_%s","节目记录id","节目记录数据","k"),
    
    PROGRAM_RECORD_FINISH("program_record_finish_%s","节目记录id","节目记录数据","k"),
    
    COUNTER_COUNT("stellaris:{captcha}:counter_count","计数器的值的key","计数器的值","k"),
    
    COUNTER_TIMESTAMP("stellaris:{captcha}:counter_timestamp","计数器的时间戳的key","计数器的时间戳","k"),
    
    VERIFY_CAPTCHA_ID("stellaris:{captcha}:verify_id_%s","校验验证码id的key","校验验证码id","k"),
    
    TICKET_USER_LIST("ticket_user_list_%s","购票人列表的key","购票人列表","k"),
    
    ACCOUNT_ORDER_COUNT("account_order_count_%s_%s","账户下订单数量的key","账户下订单数量","k"),
    
    ACCOUNT_ORDER_COUNT_ALL("account_order_count_*","账户下订单数量的key","账户下订单数量","k"),
    
    ORDER_MQ("order_mq_%s","使用mq创建的订单的订单编号","使用mq创建的订单的订单编号","k"),
    
    DISCARD_ORDER("discard_order_%s","使用mq创建方式被丢弃的订单","使用mq创建方式被丢弃的订单","k"),
    ...
    AREA_PROVINCE_LIST("area_province_list","省地区集合","省地区集合数据","k")
    ;
