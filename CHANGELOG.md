...
## 2026-09-11：建单主链路收敛为 Redis Stream + 单交易库

- Redis Lua 在同一原子操作内完成快速限购、精确锁座和 `XADD`；order-service 直接使用消费组读取 Stream，移除建单 Kafka 中继、Kafka DLT 和全局默认 Kafka 依赖。
- 新建单事务在 `stellaris_trade` 内完成请求幂等、账号节目最终限购、座位 CAS、订单及明细写入；座位行是权威库存，不再维护 `remain_number` 热计数，也不再跨服务提交节目库存。
- 支付和取消在交易库内推进订单、座位与购票计数；提交后的 Redis 展示状态由本地可靠命令重试同步。未支付取消改为 MySQL 到期扫描。
- order-service 改为普通 Hikari 单数据源并移除 ShardingSphere 路由基因；Snowflake 与 Redis worker 租约保留。migrate-service 退出默认运行主线。
- 增加 Stream PEL 重领、毒消息审计、积压准入阈值、内部接口网关隔离、单交易库 DDL、当前架构正确性脚本及容量测试夹具。
- 票档列表的 Redis 余票读取改为 pipeline 批量 `ZCARD`；多实例到期关单支持按订单主键取模分片，避免所有实例重复扫描同一批记录。
- migrate-service 增加历史模块说明，彻底移除延迟队列残留目录，并更新工程文档，使当前链路、故障边界和演进叙述与代码一致。
- 增加真实 MySQL SQL 语义验证，覆盖请求唯一键、最终限购边界、库存快照保护、座位 CAS 和多座事务回滚；修复本地压测用户物理表夹具的 SM4 密文格式及并行负载脚本的 PowerShell 语法。
- 本地 Compose 实际启动五项基础设施及五个核心服务，端到端验证重复受理、Stream 消费、单库事务、PEL 清空、身份隔离、取消与 Redis 回写全部通过。
- Maven、前端、Compose 和故障演练的实际验证结果以本次变更最终验收记录为准；旧 Kafka 性能报告只保留为历史材料。
...
- 复现确认浏览器提交中文姓名后，数据库保存为 UTF-8/系统默认字符集错误转换产生的乱码；绕过网关直调用户服务仍可复现，根因位于用户服务请求解码。
- 用户服务显式启用并强制 Servlet UTF-8 请求/响应编码；网关 JSON 请求体改为按 UTF-8 字节显式解码和重建，避免依赖 Windows/JVM 默认字符集。
- 修正一条现存乱码购票人记录，并清理对应列表缓存。
- 通过前端代理重新新增中文测试购票人并读取验证，接口脱敏和字符编码均正确；测试记录验证后已删除。
...
- 本轮只进行静态审阅和问题归档，没有修改业务代码、SQL、配置或运行环境。
...
- 前序“已执行验证”是当时链路样例记录，不构成上述并发反例已被验证或修复的证明。完成 P0 修复并新增对应异常/并发测试前，只能按“至少一次 + 业务幂等 + 最终一致性的工程化原型”描述系统能力。
...
- 当前预热是“维护态内原地重建 + ready/version 原子发布”，并非完整的版本化双缓冲 Key；它保证不清除活跃锁座和失败时不开放半成品，但预热期间不可下单。若需要零停机，需要进一步让 Intent 绑定 inventoryVersion 并延迟清理旧版本。
...
| `docs/adr/002~004` | 固化关键架构取舍及其适用边界。 |
...
| 文件或目录 | 主要修改 |
| --- | --- |
| `ops/docker-compose.local.yml` | Compose 项目、容器身份和持久卷统一为本地 Stellaris 环境，重建 MySQL、Redis、Nacos、Elasticsearch |
...
- 初始化 SQL 已由 `damai_*.sql` 全部改名为 `xingyan_*.sql`，历史扩容脚本和 Prometheus 告警也已改为星演库名、文件名、规则组及告警名。为避免一次高风险大重构，本轮仍没有重命名 Java 根包 `com.damai` 和 Maven 模块目录 `damai-*`；它们仅作为源码兼容标识保留，不会进入运行库、容器、Topic、Redis key、服务名及前端品牌。若需要源码树也完全无旧标识，应单独进行全包名/模块名迁移并重新跑全量回归。
