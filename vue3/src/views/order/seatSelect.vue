...
const getRowStyle = (rowIndex) => {
  const total = Math.max(allRows.value.length - 1, 1)
  const inset = Math.round(80 * (1 - rowIndex / total))
  return { paddingLeft: `${inset}px`, paddingRight: `${inset}px` }
}
