package com.stellaris.pay.alipay;
...
import com.stellaris.enums.AlipayTradeStatus;
...
public class AlipayStrategyHandler implements PayStrategyHandler {
    ...
    public PayResult pay(String outTradeNo, BigDecimal price, String subject, String notifyUrl, String returnUrl){
        try {
            ...
            return new PayResult(response.isSuccess(), response.isSuccess() ? "INITIATED" : "FAILED",
                    response.getBody(), response.isSuccess() ? "支付宝收银台已创建" : response.getMsg());
        }catch (Exception e) {
           log.error("alipay pay error",e);
           throw new StellarisFrameException(BaseCode.PAY_ERROR);
        }
    }
    ...
    public RefundResult refund(String outTradeNo, BigDecimal price, String reason, String refundNo) {
        ...
        if (refundNo == null || refundNo.isBlank()) {
            throw new IllegalArgumentException("stable refundNo is required");
        }
        ...
        try {
            ...
            throw new StellarisFrameException(BaseCode.REFUND_ERROR);
        }
    }
    ...
    private Integer convertPayBillStatus(String tradeStatus){
        ...
        throw new StellarisFrameException(BaseCode.ALIPAY_TRADE_STATUS_NOT_EXIST);
    }
}
