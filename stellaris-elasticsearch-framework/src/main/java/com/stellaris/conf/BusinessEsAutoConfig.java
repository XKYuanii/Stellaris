...
public class BusinessEsAutoConfig {
	...
	public RestClient businessEsRestClient(BusinessEsProperties businessEsProperties) {
		...
		builder.setHttpClientConfigCallback(httpClientBuilder -> {
			httpClientBuilder
					.setMaxConnTotal(businessEsProperties.getMaxConnectNum())
					.setMaxConnPerRoute(businessEsProperties.getMaxConnectNum())
					.setDefaultIOReactorConfig(IOReactorConfig.custom()
							// I/O selector threads are not the connection-pool size.
							.setIoThreadCount(businessEsProperties.getIoThreadCount())
							.build());
			if (Objects.nonNull(finalCredentialsProvider)) {
